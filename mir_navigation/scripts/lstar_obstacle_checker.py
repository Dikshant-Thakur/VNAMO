...
class LStarObstacleChecker(Node):
    """
    Subscribes to PointCloud2 and serves /lstar/check_area.
    - Builds/publishes ROI marker
    - Accumulates coverage grid within ROI (excluding box footprint)
    - Orients camera/EE to look at uncovered regions until threshold
    """
    ...
    def _schedule_orient_to_initial_view(self, req) -> bool:
        ...
        self.get_logger().info(
            f"[VIEW] Target MAP: ({target_x_map:.2f}, {target_y_map:.2f}) -> PLANNING: ({target_x_pl:.2f}, {target_y_pl:.2f})"
        )
        target_z_pl = 0.0
        yaw = float(req.yaw)
    ...
    def _process_job_tick(self):
        ...
        try:
            if self._job_state == "INIT":
                ok = self._schedule_orient_to_initial_view(self._job_req)
    ...
    def _on_check_area(self, req, resp):
        ...
        try:
            ...
            self._box_fp = (
                float(req.box_center_x),
                float(req.box_center_y),
                float(req.box_length),
                float(req.box_width),
            )
