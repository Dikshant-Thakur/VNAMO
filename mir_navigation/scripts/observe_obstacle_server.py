...
class FixedMarkerNavigator(Node):
    ...
    def align_camera_to_marker_pose(self, marker_pose_map: PoseStamped):
        ...
        if self.approach_yaw_deg is not None:
            ...
            if use_camera_tf:
                # Make camera optical Z point to marker
                q_des = self._lookat_cameraZ_quat(v_cam_to_marker)
            else:
    ...
    def _lookat_cameraZ_quat(self, v_cam_to_marker: np.ndarray) -> np.ndarray:
        """
        Build quaternion such that camera optical Z-axis points towards marker,
        with Y-axis roughly "up" (simple Gram-Schmidt).
        """
