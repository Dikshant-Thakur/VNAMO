...
from mir_navigation.srv import ComputeSidePeekPoints
from math import radians
from mir_navigation.srv import ComputePreManipPose
...
class PushOrchestrator(Node):
    ...
    def _handle_compute_side_peek_points(self, request, response):
        """
        Service callback:
        - planner se box info leता hai
        - _compute_side_peek_targets se 2 points nikalta hai
        - sirf points wapas bhejta hai (no navigation)
        """
    ...
    def _handle_compute_pre_manip_pose(self, request, response):
        """
        Planner se:
        - box_x, box_y, box_L, box_W
        - dir ("+X"/"-X"/"+Y"/"-Y")
        lekar, existing _premanip_pose logic se ek pre-manip pose nikalta hai.
        Sirf pose return karta hai, navigation NHI karta.
        """

        # 1) Box info ko node ke box state me set karo
        ...
        try:
            ...
            self.get_logger().warn(f"[PRE_MANIP] Failed to compute pre-manip pose: {e}")
            response.success = False
            return response
    ...
    def _compute_side_peek_targets(
        ...
        """
        Planner se aayi box info + global costmap use karke:
        1) _compute_plan() se ek free direction choose karta hai
        2) us direction ke along obstacle-side anchor nikaalta hai
        3) LEFT/RIGHT side-peek target poses (x, y, yaw) compute karta hai

        Returns:
            None -> agar koi direction free nahi mili / costmap ready nahi
            dict -> {
                'left':  (xL, yL, yaw),
                'right': (xR, yR, yaw),
            }
        ...
        # --- Safety: costmap + pose available? ---
        if self.grid is None or self.robot_pose is None:
            self.get_logger().warn("[VIS] No costmap/pose → cannot compute side-peek targets.")
            return None

        # --- Box state update so _compute_plan() also uses correct box ---
        if hasattr(self, "box"):
            self.box["x"] = box_x
            self.box["y"] = box_y
            self.box["L"] = box_L
        ...
        # --- 1) Use existing planner logic: choose corridor via _compute_plan() ---
        plan = self._compute_plan()
        if plan is None or "dir" not in plan:
            self.get_logger().warn("[VIS] _compute_plan returned None / no dir (no free corridor).")
            return None

        plan_dir = plan["dir"]   # '+X' / '-X' / '+Y' / '-Y'
        self.get_logger().info(f"[VIS] Using plan_dir={plan_dir} from _compute_plan()")

        # --- 2) Direction basis (same as _go_side_peek_obstacle_side) ---
        ...
        # --- 3) Box center (anchor near obstacle) ---
        bx, by = box_x, box_y
        ax = bx + peek_forward * fx
        ay = by + peek_forward * fy

        # --- 4) Corridor width per push axis ---
        if plan_dir in ("+X", "-X"):
            corridor_W = max(self.robot_W, box_W)
        else:  # "+Y" / "-Y"
            corridor_W = max(self.robot_L, box_L)

        # --- 5) Safe side offset (corridor edge + half robot + buffer) ---
        ...
        # --- 6) Left/Right targets facing down the corridor ---
